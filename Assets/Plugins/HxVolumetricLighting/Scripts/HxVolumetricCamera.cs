...
public class HxVolumetricCamera : MonoBehaviour
{
    ...
    TBufferDepth TransparencyBufferDepth = TBufferDepth.x8;

    //Amount of Depth slices used in 3D density texture
    BufferDepth DensityBufferDepth = BufferDepth.x12;
    //end

    public enum BufferDepth { x4 = 0, x8 = 1, x12 = 2, x16 = 3 };
    public enum TBufferDepth { x4 = 0, x8 = 1, x12 = 2, x16 = 3 };
    ...
    static void CreateShader(Shader source, int i, out Material outMaterial, bool point = true)
    {
        ...
        if (v >= 1) { v -= 1; } else { outMaterial.EnableKeyword("SHADOWS_OFF"); vc++; };
    ...
    void BuildBuffer()
    {
        ...
        if (BuffersBuilt)
        {
            ...
            if (BufferRender != null) Mycamera.RemoveCommandBuffer(RenderEvent, BufferRender);
            BuffersBuilt = false;
        }
    ...
    void OnDestroy()
    {
        ...
        if (BuffersBuilt)
        {
            if (BufferSetup != null && SetupBufferAdded) { Mycamera.RemoveCommandBuffer(SetupEvent, BufferSetup); SetupBufferAdded = false; }
            if (BufferRender != null) Mycamera.RemoveCommandBuffer(RenderEvent, BufferRender);
            if (BufferFinalize != null && FinalizeBufferAdded) { Mycamera.RemoveCommandBuffer(FinalizeEvent, BufferFinalize); FinalizeBufferAdded = false; }
#if HxApplyDirect
    ...
    void CalculateEvent()
    {
        switch (Mycamera.actualRenderingPath)
        {
            ...
            case RenderingPath.Forward:
                ...
                if (Mycamera.depthTextureMode == DepthTextureMode.Depth) { RenderEvent = CameraEvent.AfterDepthTexture; SetupEvent = CameraEvent.AfterDepthTexture; }
                if (Mycamera.depthTextureMode == DepthTextureMode.DepthNormals) { RenderEvent = CameraEvent.AfterDepthNormalsTexture; SetupEvent = CameraEvent.AfterDepthNormalsTexture; }
    ...
    void CreateLightbuffers()
    {
        ...
        for (int i = 0; i < ActiveLights.Count; i++)
        {
            ActiveLights[i].BuildBuffer(BufferRender);
        }
    }
