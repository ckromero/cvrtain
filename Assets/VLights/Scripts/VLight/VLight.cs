...
public partial class VLight : MonoBehaviour
{
    ...
    void CalculateMinMax(out Vector3 min, out Vector3 max, bool forceFrustrumUpdate)
    {
        ...
        {
            VLightGeometryUtil.RecalculateFrustrumPoints(cam, aspect, out _frustrumPoints);
        }
    ...
    Matrix4x4 CalculateProjectionMatrix()
    {
        ...
        if (!cam.orthographic)
        {
            projectionMatrix = Matrix4x4.Perspective(fov, cam.aspect, near, far);
        }
        else
        {
            float halfOrtho = cam.orthographicSize;
            projectionMatrix = Matrix4x4.Ortho(
                -halfOrtho * cam.aspect,
                halfOrtho * cam.aspect,
                -halfOrtho,
                halfOrtho,
                near, far);
    ...
    void CreateDepthTexture(LightTypes type)
    {
        ...
        else if ((type == LightTypes.Spot || type == LightTypes.Orthographic) && _depthTexture.isCubemap && _depthTexture.IsCreated())
        {
#if DEBUG_MODE
        ...
        if (_cameraHasBeenUpdated)
        {
            ...
            switch (lightType)
            {
                case LightTypes.Area:
                    ...
                    cam.orthographicSize = pointLightRadius;
                    break;
                case LightTypes.Point:
                    cam.orthographic = true;
                    cam.aspect = aspect;
                ...
                case LightTypes.Spot:
                    ...
                    cam.nearClipPlane = Mathf.Max(0.01f, spotNear);
    ...
    void SetShaderPropertiesBlock(MaterialPropertyBlock propertyBlock)
    {
        ...
        {
            case LightTypes.Point:
                propertyBlock.SetVector(_idLightParams, new Vector4(near, far, orthoSize, aspect));
                break;
            case LightTypes.Orthographic:
                propertyBlock.SetVector(_idLightParams, new Vector4(near, far, orthoSize, aspect));
                break;
